...
public class Sample {
    ...
    private static final short[] sinc_table = new short[]{
            0,
            -7,
            27,
            -71,
        ...
        if (loop_length <= 1) {
            ...
            if (!ping_pong) {
                ...
                for(int offset = 0; offset < loop_length; ++offset) {
                    short sample = data[loop_start + loop_length - offset - 1];
            ...
            for(int offset = 0; offset < 16; ++offset) {
                short sample = this.sample_data[loop_start + offset];
        ...
        for(int end = frame_offset + frames - 1 << 1; frames > 0; frames = end - offset + 2 >> 1) {
            ...
            if (max_sample_idx > loop_end) {
                while(sample_idx <= loop_end) {
                    int var19 = offset++;
                    mix_buffer[var19] += this.sample_data[sample_idx] * left_gain >> 15;
                    var19 = offset++;
                    mix_buffer[var19] += this.sample_data[sample_idx] * right_gain >> 15;
                    sample_frac += step;
        ...
        for(int end = frame_offset + frames - 1 << 1; frames > 0; frames = end - offset + 2 >> 1) {
            ...
            if (max_sample_idx > loop_end) {
                while(sample_idx <= loop_end) {
                    ...
                    int var23 = offset++;
                    mix_buffer[var23] += amplitude * left_gain >> 15;
                    var23 = offset++;
                    mix_buffer[var23] += amplitude * right_gain >> 15;
                    sample_frac += step;
                    sample_idx += sample_frac >> 15;
                    sample_frac &= 32767;
                }
                ...
                while(offset <= end) {
                    ...
                    amplitude += (this.sample_data[sample_idx + 1] - amplitude) * sample_frac >> 15;
                    int var10001 = offset++;
                    mix_buffer[var10001] += amplitude * left_gain >> 15;
                    var10001 = offset++;
                    mix_buffer[var10001] += amplitude * right_gain >> 15;
                    sample_frac += step;
                    sample_idx += sample_frac >> 15;
                    sample_frac &= 32767;
                }
